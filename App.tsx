    ...
    <div className="max-w-7xl mx-auto px-4 h-16 sm:h-20 flex items-center justify-between gap-4">
      <button onClick={() => setScreen('menu')} className="flex items-center gap-2 group">
        ...
        <span className="font-black text-slate-800 tracking-tighter uppercase italic hidden sm:block">
          ԻՍՊԱՆԵՐԵՆ: <span className="text-brand-blue">ՓԱԶԼ</span>
        </span>
      </button>
      ...
      <div className="flex items-center gap-1 sm:gap-2 md:gap-4 overflow-x-auto no-scrollbar">
        ...
        <NavButton active={currentScreen === 'puzzle_game'} icon={<Gamepad2 className="w-4 h-4 sm:w-5 sm:h-5"/>} label="Խաղ" onClick={() => setScreen('puzzle_game')} />
      </div>
    </div>
  </nav>
);
    ...
    <div className="flex flex-col items-center gap-12 sm:gap-20 py-12 sm:py-20">
      <div className="text-center space-y-4 max-w-3xl px-4">
        ...
        <p className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-400 uppercase tracking-[0.3em]">
          ԻՍՊԱՆԵՐԵՆԻ ՓԱԶԼԻ ԱՐԿԱԾՆԵՐԸ
        </p>
      </div>

      <div className="grid grid-cols-1 gap-8 w-full max-w-5xl px-4">
        <MenuCard 
          icon={<BookOpen />}
          title="Տեսություն"
          description="Սովորիր ներկա ժամանակի (Presente) խոնարհման կանոնները:"
          color="bg-brand-blue"
        ...
        <MenuCard 
          icon={<Puzzle />}
          title="Փազլ Մրցույթ"
          description="Ո՞վ ավելի արագ կհավաքի իր փազլը՝ Գոռը թե՞ Գայանեն:"
          color="bg-amber-500"
          onClick={() => setScreen('puzzle_game')}
        />
      </div>
    </div>
  );
